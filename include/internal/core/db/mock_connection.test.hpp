...
namespace fstore::test::db {
    struct mock_connection : core::db::connection {
        ...
        MOCK_METHOD(
            ext::task<core::db::object>,
            get_object,
            (const UUID::uuid& bucket_id, const UUID::uuid& object_id),
            (override)
        );

        MOCK_METHOD(
            ext::task<pg::portal<core::db::object>>,
            get_objects,
            (std::string_view portal, int batch_size),
            (override)
        );
        ...
        MOCK_METHOD(
            ext::task<core::db::remove_result>,
            remove_objects,
            (const UUID::uuid& bucket_id,
        ...
        MOCK_METHOD(
            ext::task<>,
            rename_bucket,
            (const UUID::uuid& id, std::string_view name),
            (override)
        );
